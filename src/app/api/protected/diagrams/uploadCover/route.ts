...
export async function POST(req: NextRequest) {
    try {
        ...
        const diagramId = formData.get('id') as string;
        if (!file) {
        ...
        const coverName = createCoverName(userId, diagramId);
...
const createCoverName = (userId: number, diagramId: string) => {
    return `diagram_cover/${userId}-${diagramId}`;
}